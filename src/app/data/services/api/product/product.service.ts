...
export class ProductService {
  ...
  public getProducts(): Observable<{ error: boolean; msg: string; data: any }> {
    ...
    return this.http
      .get<{ msg: string; data: any }>(API_PRODUCT_SERVICES.READ.ALL)
      .pipe(
        map((resp) => {
  ...
  public getProduct(
    ...
    return this.http
      .get<{ msg: string; data: any }>(
        `${API_PRODUCT_SERVICES.READ.SINGLE}${productId}`
      )
      .pipe(
        map((resp) => {
          response.msg = resp.msg;
  ...
  public createProduct(
    ...
    return this.http
      .post<{ msg: string; data: Product }>(
        API_PRODUCT_SERVICES.CREATE.PRODUCT,
        product
      )
      .pipe(
        map((resp) => {
  ...
  ): Observable<{ error: boolean; msg: string; data: Partial<Product> }> {
    const response: GenericResponse = { error: true, msg: '', data: null };
    console.log(2);

    return this.http
      .post<{ msg: string; data: Partial<Product> }>(
        `${API_PRODUCT_SERVICES.UPDATE.PRODUCT}${productId}`,
        product
      )
      .pipe(
        map((resp) => {
